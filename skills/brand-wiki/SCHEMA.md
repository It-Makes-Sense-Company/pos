...
depth: seed | sketch | developing | rich  # «проработанность»: сколько фактуры собрано
# --- опционально ---
...
**Depth («проработанность») и режим взаимодействия (ask → propose):**
- `seed` — тема есть только из CV/паблика, в диалоге не подтверждена. `sketch` — пара деталей.
  `developing` — есть личная фактура из разговора. `rich` — раскрыта глубоко (история, мотивация, нюансы).
- Источник `perspective: public/self` БЕЗ диалога = максимум `sketch`. `developing`/`rich` ставятся
  только после ответов субъекта в разговоре.
- **Режим агента зависит от depth × weight × gap:**
  - `gap: true` или (`weight: high` И `depth ≤ sketch`) → **режим вопросов**: агент задаёт тёплые
    уточняющие вопросы (по одному), добирая фактуру. Выводы субъекту НЕ вываливаются.
  - `depth ≥ developing` → **режим идей**: агент предлагает ракурсы, подталкивает порассуждать,
    помогает оформить тему для контента (но не пишет посты за человека).
- **Question agenda:** executive-summary + список `gap:true`/важных-тонких тем = очередь вопросов,
  отсортированная по `weight × (чем меньше depth)`. Это карта онбординга «по гвоздю в день».
- Lint выводит: важные темы с низким `depth` (что спросить дальше) и созревшие темы без идей/действий.
